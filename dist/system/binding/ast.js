...
export class AccessMember extends Expression {
    ...
    connect(binding, scope) {
        ...
        if (objectObserver) {
            observer = new PathObserver(objectObserver, value => {
...
export class AccessKeyed extends Expression {
    ...
    connect(binding, scope) {
        var objectInfo = this.object.connect(binding, scope), keyInfo = this.key.connect(binding, scope), observer = new AccessKeyedObserver(objectInfo, keyInfo, binding.observerLocator, () => this.evaluate(scope, binding.valueConverterLookupFunction));
        return {
            value: this.evaluate(scope, binding.valueConverterLookupFunction),
            observer: observer
        };
