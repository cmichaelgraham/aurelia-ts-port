...
export function bootstrap(configure) {
    return onReady(() => {
        var loader = new window.AureliaLoader(), aurelia = new Aurelia(loader);
...
function configureAurelia(aurelia) {
    return window.System.normalize('aurelia-bootstrapper').then(function (bName) {
        var toLoad = [];
        toLoad.push(window.System.normalize('aurelia-templating-binding', bName).then(templatingBinding => {
            aurelia.use.defaultBindingLanguage = function () {
                aurelia.use.plugin(templatingBinding);
                return this;
            };
        }));
        toLoad.push(window.System.normalize('aurelia-templating-router', bName).then(templatingRouter => {
            aurelia.use.router = function () {
                aurelia.use.plugin(templatingRouter);
                return this;
            };
        }));
        toLoad.push(window.System.normalize('aurelia-history-browser', bName).then(historyBrowser => {
            aurelia.use.history = function () {
                aurelia.use.plugin(historyBrowser);
                return this;
            };
        }));
        toLoad.push(window.System.normalize('aurelia-templating-resources', bName).then(name => {
        ...
        toLoad.push(window.System.normalize('aurelia-event-aggregator', bName).then(eventAggregator => {
